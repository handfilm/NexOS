...
exports.serveCatalogBundleHttp = exports.generateCatalogBundle = exports.catalogRedirect = exports.normalizeProductEvent = exports.arutemikaWebhook = exports.pollShopDriveChanges = exports.callGemini = exports.markSpecVerified = exports.recordPaymentEvent = exports.authorizeCutting = void 0;
 ...
 */
async function verifyMasterPinWithRateLimit(operatorPin, callerId, context) {
    ...
    if (!masterPin) {
        ...
        throw new https_1.HttpsError('failed-precondition', 'Security configuration error: MASTER_OPERATOR_PIN secret is not configured on the server.');
    }
    ...
    if (rateLimitSnap.exists) {
        ...
        if (failedAttempts >= 5 && Date.now() - lastFailedAt < fifteenMinutesMs) {
            ...
            throw new https_1.HttpsError('resource-exhausted', `Security lockout: Max PIN attempts exceeded. Try again in ${remainingMinutes || 15} minutes.`);
        }
    }
    // Check PIN
    if (!operatorPin || typeof operatorPin !== 'string' || operatorPin !== masterPin) {
        ...
        throw new https_1.HttpsError('permission-denied', 'Invalid Master PIN authorization.');
    }
 ...
 */
exports.authorizeCutting = (0, https_1.onCall)({
    region: 'asia-east1',
    cors: true,
    maxInstances: 10,
    ...
    if (!orderId || typeof orderId !== 'string') {
        throw new https_1.HttpsError('invalid-argument', 'A valid "orderId" string is required.');
    }
    if (!idempotencyKey || typeof idempotencyKey !== 'string') {
        throw new https_1.HttpsError('invalid-argument', 'A valid "idempotencyKey" UUID string is required.');
    }
    ...
    try {
        const result = await db.runTransaction(async (transaction) => {
            ...
            if (idemSnap.exists) {
                ...
                if (cached.orderId && cached.orderId !== orderId) {
                    throw new https_1.HttpsError('invalid-argument', 'Idempotency key scope violation.');
                }
            ...
            if (!orderSnap.exists) {
                throw new https_1.HttpsError('not-found', `Order document "${orderId}" was not found.`);
            }
            ...
            const totalAmount = Number(orderData.totalAmount ??
                ...
                0);
            const runningTotalPaid = Number(orderData.runningTotalPaid ??
            ...
            if (runningTotalPaid < requiredAdvance) {
                ...
                throw new https_1.HttpsError('failed-precondition', `Advance balance below 50% threshold. Confirmed running paid: ${runningTotalPaid}, required 50%: ${requiredAdvance}.`);
            }
            // Invariant 2: Spec Verification Gate (Requires "verified_by_human")
            const specStatus = orderData.specVerification?.status ||
                orderData.techPack?.status ||
                orderData.techPackStatus;
            if (specStatus === 'pending_ai_review') {
                throw new https_1.HttpsError('failed-precondition', 'Specs must be explicitly confirmed before cutting. Tech-pack status is currently "pending_ai_review".');
            }
            if (specStatus !== 'verified_by_human') {
                throw new https_1.HttpsError('failed-precondition', `Specs must be explicitly confirmed before cutting. Tech-pack spec verification status must be "verified_by_human" (found: "${specStatus || 'unverified'}").`);
            }
        ...
        throw new https_1.HttpsError('internal', error.message || 'Internal atomic transaction error.');
    }
});
/**
 * ======================================================================
 ...
 */
exports.recordPaymentEvent = (0, https_1.onCall)({
    region: 'asia-east1',
    cors: true,
    maxInstances: 10,
    secrets: ['MASTER_OPERATOR_PIN']
    ...
    if (!validMethods.includes(method)) {
        throw new https_1.HttpsError('invalid-argument', `Invalid payment method "${method}". Allowed values: ${validMethods.join(', ')}`);
    }
    if (!referenceId || typeof referenceId !== 'string') {
        throw new https_1.HttpsError('invalid-argument', 'A valid string "referenceId" is required.');
    }
    ...
    try {
        const result = await db.runTransaction(async (transaction) => {
            ...
            const orderSnap = await transaction.get(orderRef);
            if (!orderSnap.exists) {
                throw new https_1.HttpsError('not-found', `Order "${orderId}" does not exist.`);
            }
            const orderData = orderSnap.data() || {};
            const totalAmount = Number(orderData.totalAmount ??
                orderData.total ??
                orderData.grandTotal ??
                0);
            const currentRunning = Number(orderData.runningTotalPaid ??
                orderData.totalAmountPaid ??
                orderData.amountPaid ??
                0);
        ...
        throw new https_1.HttpsError('internal', err.message || 'Error executing payment ledger transaction.');
    }
});
/**
 * ======================================================================
 ...
 */
exports.markSpecVerified = (0, https_1.onCall)({
    region: 'asia-east1',
    cors: true,
    maxInstances: 10,
    secrets: ['MASTER_OPERATOR_PIN']
    ...
    try {
        ...
        throw new https_1.HttpsError('internal', err.message || 'Error verifying tech pack specs.');
    }
});
/**
 * ======================================================================
...
exports.callGemini = (0, https_1.onCall)({
    ...
    // Authenticate Caller
    if (!request.auth) {
        throw new https_1.HttpsError('unauthenticated', 'Authentication required to invoke Hands & Head Gemini proxy.');
    }
    const { prompt, audioBase64, mimeType = 'audio/webm', systemInstruction, jsonOutput = false, responseSchema } = request.data || {};
    if (!prompt && !audioBase64) {
        throw new https_1.HttpsError('invalid-argument', 'Either "prompt" (string) or "audioBase64" must be provided.');
    }
    const apiKey = process.env.GEMINI_API_KEY;
    if (!apiKey) {
        throw new https_1.HttpsError('failed-precondition', 'Server configuration error: GEMINI_API_KEY environment secret is not set.');
    }
    ...
    try {
        ...
        throw new https_1.HttpsError('internal', err.message || 'Gemini inference failed upstream.');
    }
});
/**
 * ======================================================================
 ...
 * ======================================================================
 */
var drive_ingestion_1 = require("./nexos-sync/drive-ingestion");
Object.defineProperty(exports, "pollShopDriveChanges", { enumerable: true, get: function () { return drive_ingestion_1.pollShopDriveChanges; } });
var arutemika_webhook_1 = require("./nexos-sync/arutemika-webhook");
Object.defineProperty(exports, "arutemikaWebhook", { enumerable: true, get: function () { return arutemika_webhook_1.arutemikaWebhook; } });
var normalizer_1 = require("./nexos-sync/normalizer");
Object.defineProperty(exports, "normalizeProductEvent", { enumerable: true, get: function () { return normalizer_1.normalizeProductEvent; } });
var redirect_service_1 = require("./nexos-sync/redirect-service");
Object.defineProperty(exports, "catalogRedirect", { enumerable: true, get: function () { return redirect_service_1.catalogRedirect; } });
var bundle_generator_1 = require("./nexos-sync/bundle-generator");
Object.defineProperty(exports, "generateCatalogBundle", { enumerable: true, get: function () { return bundle_generator_1.generateCatalogBundle; } });
Object.defineProperty(exports, "serveCatalogBundleHttp", { enumerable: true, get: function () { return bundle_generator_1.serveCatalogBundleHttp; } });
