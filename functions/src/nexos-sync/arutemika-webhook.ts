...
const logger = new NexosLogger('ArutemikaWebhook');
 ...
 */
export const arutemikaWebhook = onRequest(
  {
    region: 'asia-east1',
    cors: true,
    maxInstances: 20,
  ...
  async (req, res) => {
    ...
    const incomingProducts: any[] = Array.isArray(body)
      ? body
    ...
    for (const [index, rawProduct] of incomingProducts.entries()) {
      ...
      try {
        const publishResult = await publishRawProductEvent(event);
