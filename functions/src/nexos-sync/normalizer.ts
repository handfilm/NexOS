...
import {
  ...
  ProvenanceBadge,
  CatalogOrigin,
  PrivateCatalogMetadata,
} from './types';
import { NexosLogger } from './logger';

const logger = new NexosLogger('ProductNormalizer');
 ...
 */
export function calculateWholesaleTiers(retailPrice: number): [WholesaleTier, WholesaleTier, WholesaleTier] {
 ...
 */
export function deriveProvenanceAndMoq(
  origin: CatalogOrigin,
  materials: string[],
  category: string,
  retailPrice: number
): { provenance: ProvenanceBadge; moq: number; leadTimeDays: number; hsCode: string } {
  ...
  if (isLeather) {
    ...
    const provenance: ProvenanceBadge = {
 ...
 */
export function transformToB2BProduct(
  origin: CatalogOrigin,
  raw: Record<string, any>,
  existingSyncVersion = 0
): { product: B2BProduct; privateMetadata: PrivateCatalogMetadata } {
  let sku = '';
  let title = '';
  let description = '';
  ...
  if (origin === 'arutemika.com') {
    ...
    if (Array.isArray(raw.images)) {
      images = raw.images.map((img: any) => (typeof img === 'string' ? img : img.src)).filter(Boolean);
    }
    if (raw.leather_type) {
  ...
  const { provenance, moq, leadTimeDays, hsCode } = deriveProvenanceAndMoq(
    origin,
    materials,
    category,
    retailPrice
  ...
  const privateMetadata: PrivateCatalogMetadata = {
    catalogId,
    origin,
 ...
 */
export const normalizeProductEvent = onMessagePublished(
  {
  ...
  async (event) => {
    ...
    let productEvent: RawProductEvent;
    try {
    ...
    try {
      ...
      const committedProduct = await db.runTransaction(async (transaction) => {
        ...
        let originalCreatedAt: any = admin.firestore.FieldValue.serverTimestamp();
        ...
        const { product, privateMetadata } = transformToB2BProduct(
          origin,
          rawPayload,
          existingVersion
        );
