/* ═══════════════════════════════════════════════════════════════
   Hands & Head — js/qrcode-generator.js
   Standalone Vector & Canvas QR Code Engine (ISO/IEC 18004)
   No external dependencies · 100% Offline & Instant
   ═══════════════════════════════════════════════════════════════ */
...
(function (root, factory) {
  ...
  /* ── Public API & SVG/Canvas Renderers ── */
  function generateSvg(text, options) {
    options = options || {};
    var size = options.size || 200;
    var margin = options.margin !== undefined ? options.margin : 2;
    var darkColor = options.darkColor || '#0E121B';
    var lightColor = options.lightColor || '#FFFFFF';
    var level = options.level || 'M';

    var qr = new QRCodeModel(0, QRErrorCorrectLevel[level] !== undefined ? QRErrorCorrectLevel[level] : QRErrorCorrectLevel.M);
    qr.addData(text);
    qr.make();
    ...
    return '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ' + size + ' ' + size + '" width="' + size + '" height="' + size + '" style="display:block;shape-rendering:crispEdges;">' +
      (lightColor ? '<rect width="' + size + '" height="' + size + '" fill="' + lightColor + '"/>' : '') +
      '<path d="' + paths.join(' ') + '" fill="' + darkColor + '"/>' +
      '</svg>';
  }

  function renderToCanvas(text, canvas, options) {
    options = options || {};
